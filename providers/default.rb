...
def shared_dir
  if new_resource.shared_dir
    new_resource.shared_dir
  else
    ::File.join(root_directory, '/shared')
  end
end
...
action :create do
  ...
  converge_by("Create app root dir #{new_resource.directory}") do
  ...
  converge_by("Create shared root dir #{new_resource.sidekiq_dir}") do
    directory "#{::File.join(root_directory, 'shared')}" do
      owner new_resource.owner if new_resource.owner
      group new_resource.group if new_resource.group
      mode '0755'
      recursive true
  ...
  converge_by("Create logroate config #{new_resource.name}") do
    ...
    logrotate_app "sidekiq-#{new_resource.name}" do
      ...
      options %w(missingok compress copytruncate delaycompress notifempty dateext)
